const characters = [
    ...
    { id: 37, charName: "Jean", HP: 50, Str: 20, Mag: 20, Dex: 35, Spd: 40, Def: 25, Res: 20, Lck: 25, Bld: 5, image: "images/char-sprite/jean.png" }
]
...
const tableBody = document.querySelector(".growths")

function addGrowths(charGrowth, classGrowth) {
    return charGrowth + classGrowth;
}

function updateHeader(characterId, classId) {
...
const builderMode = document.querySelector("#builderMode");
const team = [];

function isOnTeam(charId, classId) {
    return team.some(member => member.charId == charId && member.classId == classId);
}
...
function updateTable() {
    ...
    updateHeader(characterId, classId);

    if (char && charClass) {
        let signCell = "";

        if (builderMode.value == "on") {
        const onTeam = isOnTeam(characterId, classId);
        const icon = onTeam ? "minus.png" : "plus.png";
        const altText = onTeam ? "Remove unit" : "Add unit";
    ...
    const sign = row.querySelector(".signIcon");
    if (sign) {
        sign.addEventListener("click", function () {
            const charId = parseInt(this.dataset.charid);
            const classId = parseInt(this.dataset.classid);

            if (isOnTeam(charId, classId)) {
                const index = team.findIndex(m => m.charId == charId && m.classId == classId);
                team.splice(index, 1);
            }
            else {
                team.push({charId, classId});
            }
...
if (builderMode) {
    builderMode.addEventListener("change", updateTable);
}
